...
final class PaymentIdentificationType extends \Df\Config\Source {
	...
	protected function map() {return [
		self::$ORDER_IID => 'Order Increment ID', self::$AUTOGENERATED => self::$AUTOGENERATED_V
	];}
	 ...
	 * @throws \Exception|LE
	 */
	static function id(O $o) {
		/** @var S $s */
		$s = dfps($o);		
		/** @var string $result */
		if (self::$AUTOGENERATED === $s->paymentIdentificationType()) {
			// 2016-08-27 Метод должен возвращать неизменное значение для конкретного заказа.
			/** @var array(string => string) $cache */
			static $cache;
			/** @var string $key */
			$key = $o->getIncrementId();
			if (!isset($cache[$key])) {
				$cache[$key] = df_uid(10);
			}
			$result = $cache[$key];
		...
		else if (!self::check($result = $o->getIncrementId())) {
			df_error(__(
				...
				. "backend option."
				, $result, __(self::$AUTOGENERATED_V)
			));
		}
		return $result;
	}
